...
public class Board {
    ...
    public boolean columnFull(int col) {
        ...
        if (!columnFull(i)) { // check if the column is full by just checking the 0'th row's value) {
            return false;
        }
        return true;
        }
    ...
    public boolean checkIfPlayerIsTheWinner(String playerNumber) {
    	return checkHorizontal(playerNumber) || checkVertical(playerNumber) ||
                checkLeftDiagonal(playerNumber) || checkRightDiagonal(playerNumber);
    ...
    public boolean checkHorizontal(String playerNumber) {
        for (int col = 0; col < board[0].length - 3; col++) {
           ...
           // Check for horizontal win
            for (int col1 = 0; col < board[0].length - 3; col++) {
                if (board[col][col].equals(playerNumber) &&
                    board[col][col + 1].equals(playerNumber) &&
                    board[col][col + 2].equals(playerNumber) &&
                    board[col][col + 3].equals(playerNumber)) {
                    return true; 
        }
            }
        }
